...
class FlattenObservationSpace(AuctionEnv):
    ...
    def observation_space(self, agent):
        ...
        for k,v in space.items():
            low=min(low,v.low[0])
            high=max(high,v.high[0])
            shape += v.shape[0]
            # pass
                
        return Box(low,high,shape=(shape,),dtype=np.float32)
    
    def reset(self,seed=None, options=None):
        observations, _ = super().reset(seed=seed, options=options)
        # flatten_obs(observations)
        new_obs = {i : np.array([]) for i in observations.keys()}
    ...
    def step(self, actions):
        ...
        return new_obs, rewards, terminations, truncations,infos
...
class FlattenObsSingleAgent(AuctionEnv):
    ...
    def step(self, actions):
        ...
        # print(new_obs,rewards,terminations,truncations,infos,"env params")
        return new_obs[0], rewards[0], terminations[0], truncations[0],infos[0]
    




# class FlattenObsActSingleAgent(AuctionEnv):
...
#         action_space = Box(low=np.tile(np.array([self.min_bid_price]),(self.no_of_bids,1)), high=np.tile(np.array([self.max_bid_price]),(self.no_of_bids,1)), dtype=np.float32)
#         return action_space
